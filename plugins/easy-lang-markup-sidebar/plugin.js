...
tinyMCE.PluginManager.add('languageSidebar', function (editor) {
    ...
    const langColors = { en: '#eee', 'en-us': '#ddd', es: '#E6B0AA', fr: '#AED6F1', it: '#ABEBC6', de: '#F9E79F' }
    const colorsAvailable = ['#02bfe7', '#59C879', '#f9c642', '#00A3CF', '#3B8350', '#fdb81e', '#aeb0b5', '#EC0000']
    ...
    function detectDefaultLangOfPageHoldingEditor() {
        ...
        defaultLang = cleanLangAttr(editorDocument.body.children[0].getAttribute('lang'));
        }
    
        // 3. Check if the document body has a lang attribute
        if (!regexValidLangValue.test(defaultLang) && editorDocument.body.hasAttribute('lang')) {
        ...
        if (!regexValidLangValue.test(defaultLang) && topDocument.documentElement.hasAttribute('lang')) {
        defaultLang = cleanLangAttr(topDocument.documentElement.getAttribute('lang'));
        }
    
        // 5. Fallback: Use browser language or default to 'en'
        if (!regexValidLangValue.test(defaultLang)) {
        defaultLang = baseLanguage(window.navigator.language || window.navigator.userLanguage) || 'en';
        }
    ...
    function getDocumentDefaultLang() {
        ...
        if (editorBody && editorBody.children.length === 1 && editorBody.firstElementChild.hasAttribute('lang')) {
    ...
    function analyzeEditorDocumentLangUsage() {
        ...
        const langMatches = [...docContainer.innerHTML.matchAll(/\slang="(.+?)"/g)];
...
function getDocumentElementLang(el) {
     ...
     */
    function openLanguageSelectionDialog(editor, langAtts, currentLang, callBack) {
        ...
        body: {
            ...
            tabs: [
            {
                ...
                {
                    type: 'htmlpanel',
                    html: `<div style="margin-bottom:10px">Current default language: ${langAtts[currentLang] || currentLang}</div>`
                },
                {
                    type: 'selectbox',
                    name: 'language',
            ...
            },
            {
                ...
                {
                    type: 'htmlpanel',
                    html: `<div style="margin-bottom:10px">Current language: ${langAtts[currentLang] || currentLang}</div>`
                },
                {
                    type: 'input',
                    name: 'manualLanguage',
        ...
        initialData: {
            language: currentLang.toLowerCase() || defaultLangOfPageHoldingEditor.toLowerCase()
        },
        buttons: [
            {
            type: 'cancel',
        ...
        onSubmit(api) {
            ...
            let newLang = currentTab === 'listTab2' ? data.manualLanguage : data.language;
            ...
            alert('Enter a valid language code with no spaces. Or, press cancel.');
            }
        }
        });
    }
    ...
    const openConfigureLanguagesOnSelectbox = (langMenuItems, callBack) => {
      ...
      langMenuItems?.forEach((lang, index) => {
        languageChoiceItems.push(addLanguageOption(index + 1, lang, Object.prototype.hasOwnProperty.call(langAtts, lang)));
      });
          ...
          for (let i = 1; i <= 6; i++) {
            ...
            if (selectValue === '-o-') {
              if (!regexValidLangValue.test(inputValue)) {
                alert(`Enter a valid language for language ${i}. Found: '${inputValue}'`);
                return;
              }
    ...
    function revealLangMarkUp() {
      ...
      langElements.forEach((el) => {
        ...
        if (langFound && !languagesFound.hasOwnProperty(langFound)) {
          languagesFound[langFound] = availableColors.length > 0 ? availableColors.shift() : defaultColor;
        }
      });
    ...
    function moveSiblingsIntoElement(targetElement) {
      ...
      while (targetElement.previousSibling) {
        targetElement.insertBefore(targetElement.previousSibling, targetElement.firstChild);
      }
    }
    ...
    function removeLangMarkupAtCursor() {
      ...
      if (selection && selection.rangeCount === 1) {
        ...
        if (range.collapsed) {
          let element = range.startContainer.nodeType === 3 ? range.startContainer.parentElement : range.startContainer;
          ...
          if (element) {
            ...
            if (element.nodeName.toLowerCase() === 'span' && isRemovableSpan(element)) {
    ...
    function removeAllLangSpans() {
      if (!confirm('Really remove all language markup from the document?')) return;
    ...
    function removeAllChildNodes(parent) {
      if (!(parent instanceof Element)) {
        console.error('Invalid parent element provided to removeAllChildNodes:', parent);
        return;
      }
   ...
   /**
    ...
    function createLangButton(lang) {
      ...
      const buttonStyle = 'padding: 2px 8px; margin-right:3px; margin-bottom:3px; border: 1px solid #ccc';
     ...
     */
    function addButtonFocusIndicationHandler(button, focusStyle = '1px solid #1B75BC', blurStyle = '1px solid #ccc') {
    ...
    function getIconButton(iconName, label) {
      ...
      const iconButton = createElement(
          ...
          iconHTML,
          {
              ...
              title: label,
              'aria-label': label,
              style: 'align-items: center; vertical-align: middle; border: 1px solid white'
          },
          { html: true }
      );
    ...
    function createSidebarHeader(title) {
        return createElement('h2', title, {
            style: 'font-weight: bold; font-size: 1.0em; line-height: 1.4; padding: 2px; border-bottom: thin solid #ccc; text-align: center; margin: 0px'
        }, { html: true });
    }
    ...
    function createDefaultLanguageSection() {
        const div = createElement('div', null, {
            style: 'margin: 3px 3px 2px 3px; border: thin solid #ccc; border-radius: 5px;'
        });
    
        const header = createElement('div', null, {
            style: 'display: flex; flex-direction: row; align-items: center; border-bottom: thin solid #ccc'
        });
        header.appendChild(createElement('h3', 'Default doc. language', {
            style: 'font-weight: bold; font-size: 0.85em; line-height: 1.4; padding: 0px 0px 1px 4px; width: 100%'
        }));
    ...
    function getCurrentLangFromEditor() {
        const editorBody = editor.getDoc().body;
        let currentLang = '';
        if (editorBody.children.length === 1 && editorBody.firstElementChild.hasAttribute('lang')) {
            currentLang = cleanLangAttr(editorBody.firstElementChild.getAttribute('lang')) || currentLang;
        }
        return currentLang;
    }
    ...
    function createCurrentLanguageSection() {
        ...
        div.appendChild(createElement('h3', 'Current text language', {
            style: 'font-weight: bold; font-size: 0.85em; line-height: 1.4; padding: 0px 0px 1px 4px; border-bottom: thin solid #ccc'
        }));
    ...
    function createLanguageButtonsSection() {
        ...
        header.appendChild(createElement('h3', 'Language Buttons', {
            style: 'font-weight: bold; font-size: 0.85em; line-height: 1.4; padding: 0px 0px 1px 4px; width: 100%'
        }));
        ...
        if (icon) {
            icon.style.marginRight = '4px';
            header.appendChild(icon);
            icon.addEventListener('click', () => { configureLanguageButtons(); });
        }
    ...
    function createToggleViewLangMarkupButton() {
        ...
        if (iconHTML && iconHTML.startsWith('<svg')) {
            ...
            toggleViewLangMarkupButton = createElement('button', buttonHTML, {
                type: 'button',
                class: 'tox-button tox-button--secondary',
                style: 'padding: 3px 12px; margin: 3px 3px 2px 3px; align-items: center; vertical-align: middle'
            }, { html: true });
            toggleViewLangMarkupButton.firstElementChild.setAttribute('role', 'presentation');
            toggleViewLangMarkupButton.firstElementChild.style.verticalAlign = 'middle';
        } else {
    ...
    function createRemoveLangButtons() {
        ...
        fragment.appendChild(createRemoveLangButton('remove', 'Remove current lang', removeLangMarkupAtCursor));
    
        // Create and append the remove all language markup button.
        fragment.appendChild(createRemoveLangButton('warning', 'Remove all lang markup', removeAllLangSpans));
    ...
    function createRemoveLangButton(iconName, label, callback) {
        ...
        if (iconHTML && iconHTML.startsWith('<svg')) {
            ...
            button = createElement('button', buttonHTML, {
                type: 'button',
                class: 'tox-button tox-button--secondary',
                style: 'padding: 3px 12px; margin: 3px 3px 2px 3px; align-items: center; vertical-align: middle'
            }, { html: true });
        ...
        button.addEventListener('click', () => { editor.undoManager.transact(callback); });
        addButtonFocusIndicationHandler(button);
        return button;
    }
    ...
    function updateDefaultDocLangStatus() {
      ...
      try {
          ...
          if (currentDefaultDocLang) {
              defaultLangHolderEl.innerText = `${currentDefaultDocLang} ${langDescription ? ' - ' + langDescription : ''}`;
    ...
    function updateCurrentLangStatus (el) {
      ...
      if (currentLangHolderEl && defaultLangHolderEl) {
        try {
          currentLangHolderEl.innerText = currentLang ? currentLang + (langAtts[currentLang.toLowerCase()] ? ' - ' + langAtts[currentLang.toLowerCase()] : '') : '- Not Set -'
          currentLangHolderEl.style.backgroundColor = currentLang ? '#C1FFC1' : '#FF9191'
          if (currentLangIsDefault) {
            const isDefaultSpan = createElement('span', ' (from default)', { style: 'font-style: italic;' })
            currentLangHolderEl.appendChild(isDefaultSpan)
          }
          if (el.tagName !== 'BODY' && !(el.parentElement.tagName === 'BODY' && el.tagName === 'DIV')) lastDocElement = el
        } catch (ex) {}
      }
    }
  ...
  editor.ui.registry.addSidebar('mysidebar', {
      ...
      onShow: function (api) {
        sidebarIsOpen = true
        api.element().style.maxWidth = (api.element().firstChild.offsetWidth.toString() * 1.05) + 'px'
        renderLanguageButtonsInSidebar()
      },
      onHide: function (api) {
        sidebarIsOpen = false
